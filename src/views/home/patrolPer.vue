<template>
  <div id="patrolPer" v-show="homeShow">
    <div class="top">
      <div class="left fl">
        <p class="title">
          电子卷宗制作与质检总体统计报表（全省）
          ...
          >查看更多</span>
          <span class="fr ckdgbtn" @click="searchMore(1)" v-if="tableData.length>10">查看更多</span>
        </p>
        <div id="xcchart1">
        ...
        <div class="botSearch">
          <el-form :inline="true" :model="xcparam" class="demo-form-inline">
            ...
            <el-form-item label>
              ...
              >巡查</el-button>
              <el-button type="primary" @click="printOut">生成报告</el-button>
            </el-form-item>
          </el-form>
        </div>
        <div class="botlist">
...
export default {
  ...
  computed: {
    ...mapGetters(["token", "userInfo", "chart1FlagShow", "chartMOre"]),
    classOption() {
      return {
        step: 0.3,
        limitMoveNum: 5
  ...
  methods: {
    ...
    downloadResult(name) {
      let canvasID = document.body.querySelector("#home");
    ...
    gobackChart() {
      xcDrawChart1(this.xcChart1Datas, this.xcparam);
      this.$store.dispatch("user/changeChartFlag", { data: true });
    },
    handleCloseMore() {
    ...
    async searchMore(id) {
      ...
      if (id === 0) {
        ...
        setTimeout(() => {
          xcDrawChart1More(this.chartMOre);
        }, 1000);
      } else if (id == 1) {
        this.moreeTile = "电子卷宗制作与质检总体统计报表";
        this.moreChart1Flag = true;
    ...
    saveTime() {
      ...
      this.failCaseListParam.laDateEnd = this.xcparam.laDateEnd;
    },
    selStart() {
    ...
    selTime(id) {
      ...
      // console.log(this.failCaseListParam);
    },
    handleClose() {
      this.dialogVisible = false;
      this.xcparam.laDateStart = "2017-01-01";
      this.xcparam.laDateEnd = parseTime(new Date(), "{y}-{m}-{d}");
      this.failCaseListParam.laDateStart = "2017-01-01";
      this.failCaseListParam.laDateEnd = parseTime(new Date(), "{y}-{m}-{d}");
    ...
    async getFailCaseList() {
      ...
      this.allAreaList = data;
      try {
        for (let index = 0; index < self.allAreaList.length; index++) {
          const element = self.allAreaList[index];
          self.failCaseListParam.courtId = element.courtId;
          const { data } = await getFailCaseList(self.failCaseListParam);
          self.failCaseList = data;
          if (self.failCaseList.length > 0) {
            break;
          }
        }
    ...
    changeData() {
      this.$emit("changeData", "/home/patrolPer");
    },
    async getHgTop10() {
      let self = this;
    ...
    async getCaseStatistics() {
      ...
      try {
        ...
        self.tableData.forEach((element, index) => {
          ...
          self.xcChart3Datas.cityNames.push(element.courtName);
          self.xcChart5Datas.citySource.push(
            element.uploadRate.substring(0, element.uploadRate.length - 1) * 100
          );
          self.xcChart5Datas.cityNames.push(element.courtName);
        ...
        xcDrawChart1(self.xcChart1Datas, self.xcparam);
...
#patrolPer .botSearch .el-input__inner {
  background-color: #0c1f7f;
  height: 30px;
  line-height: 30px;
...
#patrolPer .botSearch .el-input {
  height: 30px;
  line-height: 30px;
}
#patrolPer .botSearch .el-input__icon {
  line-height: 25px;
}
