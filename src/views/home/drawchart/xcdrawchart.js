...
import { getCaseStatistics, getCourtList } from "@/api/quality";
...
export let chart1Flag = true;
export let linshiDatas = [];

export function xcDrawChart1(xcChart1Data, paramsData) {
    let newParams;
    newParams = JSON.parse(JSON.stringify(paramsData))
    if (xcChart1Data.length > 10) {
        xcChart1Data = xcChart1Data.slice(0, 10)
    }
    ...
    myChart.on("click", function (params) {
        if (store.getters.chart1FlagShow) {
            let courts = []
            let courtsIds = []
            let moreChart1Dtaas = []
            getCourtList({ courtId: params.data.areaId }).then(res => {
                courts.push(res.data);
                courts = courts.concat(res.data.children);
                for (let index = 0; index < courts.length; index++) {
                    const element = courts[index];
                    courtsIds.push(element.courtId);
                }
                newParams.courtIdStrings = courtsIds.join("-");
                newParams.queryFlag = 0;
                getCaseStatistics(newParams).then(res => {
                    ...
                    xcDrawChart1(moreChart1Dtaas, newParams)
                    chart1Flag = false;
                    store.dispatch('user/changeChartFlag', { data: false })
                    store.dispatch('user/changeChartMOre', { data: moreChart1Dtaas })
                    params = oldParams
                }).catch(err => {
                    console.log(err)
                })
