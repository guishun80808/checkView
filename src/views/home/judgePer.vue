          ...
          <div class="subleft fl">
            ...
            <ul class="fl">
              <li v-for="(item,index) in zhTop10List" :key="index" v-if="index < 10">
                ...
                <span class="areaname">{{item.cbrName}}</span>
            ...
            <vue-seamless-scroll :data="failListNum" :class-option="classOption" class="warp">
              <ul class="item">
                <li v-for="(item,index) in failListNum" :key="index">
                  <span class="title">{{item.cbrName}}</span>
...
export default {
  ...
  methods: {
    ...
    async getFailCaseList() {
      ...
      self.failCaseListParam.courtRoomId = this.user.courtRoomId;
      if (self.userInfo.userCode) {
        self.xcparam.cbrIdStrings = self.userInfo.userCode;
      } else {
        self.failCaseList = [];
        return;
      }
    ...
    changeData() {
      this.$emit("changeData", "/home/judgePer");
    },
    async getHgTop10() {
      let self = this;
    ...
    async getCaseStatistics() {
      ...
      this.xcparam.courtRoomIdStrings = this.userInfo.courtRoomId;
      if (this.userInfo.userCode) {
        this.xcparam.cbrIdStrings = this.userInfo.userCode;
      } else {
        this.nodata = true;
        return;
      }
      this.xcparam.queryFlag = 2;
      this.xcChart1Datas = [];
      this.xcBuHeGeDatas = [];
      this.xcChart2Datas = [
        { value: 0, name: "合格数量" },
      ...
      try {
        ...
        self.tableData.forEach((element, index) => {
          ...
          self.xcChart3Datas.cityNames.push(element.cbrName);
          self.xcChart5Datas.citySource.push(
            element.uploadRate.substring(0, element.uploadRate.length - 1) * 100
          );
          self.xcChart5Datas.cityNames.push(element.cbrName);
    ...
    async getCountTrend() {
      ...
      this.cxchatrparam6.courtRoomId = this.userInfo.courtRoomId;
...
#judgePer .botSearch .el-input__inner {
  background-color: #0c1f7f;
  height: 30px;
  line-height: 30px;
...
#judgePer .botSearch .el-input {
  height: 30px;
  line-height: 30px;
}
#judgePer .botSearch .el-input__icon {
  line-height: 25px;
}
